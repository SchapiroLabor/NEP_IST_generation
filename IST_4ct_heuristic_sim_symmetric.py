### Cell type assignment for 4 cell types with heuristic approach, symmetric dataset
...
for i in i:
    ...
    # Again, these are the 3 different proximity likelyhood matrices I used for simulating random, weak or strong self preference ct 0.
    ...
    neighborhood_probabilities = np.array(([0.45, 0.18, 0.18, 0.19],
                                        [0.18, 0.27, 0.28, 0.27],
                                        [0.18, 0.28, 0.27, 0.27],
                                        [0.19, 0.27, 0.27, 0.27]))
    ...
    cell_assignments = assign_labels.heuristic_assignment(graph, cell_type_probabilities, neighborhood_probabilities, mode='graph', dim=1000, position_dict=position_dict, grid_size=200, revision_iters=5000, n_swaps=3000)

    B = build_assignment_matrix(cell_assignments, n_cell_types=4)

    # get visualization
    viz.make_vor(1000, cell_assignments, position_dict, 4, './visualization_', f"selfweak_{i}", [x for x in range(0, C.shape[0])])
    ...
    df.to_csv(f"annotated_data/ran_ab0_0.05_{i}.csv", index=False)
