...
NH_PROBABILITIES = {
    ...
    "weak_pref": np.array([[0.18, 0.45, 0.18, 0.19],
                            [0.28, 0.18, 0.27, 0.27], 
                            [0.27, 0.19, 0.27, 0.27],
                            [0.27, 0.18, 0.28, 0.27]]),
    "strong_pref": np.array([[0.13, 0.6, 0.13, 0.14], 
                            [0.29, 0.13, 0.29, 0.29], 
                            [0.29, 0.13, 0.29, 0.29],
                            [0.29, 0.14, 0.29, 0.28]])
}
...
def simulate_tissue(scaffold_id, ct_prob, nh_prob, output_folder):
    ...
    if args.nh_prob == "random":
        ...
        if args.nh_prob == "weak_pref":
            df.to_csv(f"{output_folder}/cross01_0.45_ab0_{args.ct_prob}_{scaffold_id}.csv", index=False)
        else:
            df.to_csv(f"{output_folder}/cross01_0.6_ab0_{args.ct_prob}_{scaffold_id}.csv", index=False)
